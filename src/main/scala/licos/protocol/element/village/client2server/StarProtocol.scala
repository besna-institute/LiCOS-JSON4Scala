...
final case class StarProtocol(
  ...
  private lazy val json: Option[JsonStar] = {
    StarProtocol4Logger(village, serverTimestamp, clientTimestamp, isMarked, myCharacter, myRole, Nil).json
  }
  ...
  def forLogger(extensionalDisclosureRange: Seq[StatusCharacterProtocol]): StarProtocol4Logger = {
    StarProtocol4Logger(
      village:                    VillageInfo,
      serverTimestamp:            OffsetDateTime,
      clientTimestamp:            OffsetDateTime,
